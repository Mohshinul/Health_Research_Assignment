...
zip_file <- "/Users/mohshinulkarim/Documents/Dal/Health Research/Week 3/lab3_chest_xray.zip"

# Specify the destination directory to extract the files
destination_dir <- "/Users/mohshinulkarim/Documents/Dal/Health Research/Health_Research_Assignment"
...
**Pneumonia:** Pneumonia is an infection that inflames the air sacs, which may fill with fluid or pus in the lungs. It causes symptoms such as a cough with phlegm or pus, fever, chills, and difficulty breathing. Various organisms, such as bacteria, viruses, and fungi, can cause pneumonia.

**Benefits of Identifying Pneumonia from X-Rays Automatically:**
  **1. Early Diagnosis and Treatment:**
      **a)	Quick Diagnosis:** Finding out the problem is crucial for patients' recovery. Automated systems can quickly identify signs of pneumonia in chest X-rays, leading to faster diagnosis and treatment. 
      **b)	Decreasing Misdiagnosis:** Automated detection helps to reduce the chances of misdiagnosis and ensures that patients receive the correct treatment quickly.
  **2. Resource Optimization:**
      **a)	Radiologist Support:** Automated systems can assist radiologists by pre-screening X-rays.  So they can focus on more complex cases and reduce their workload.
      **b)	Cost Efficiency:** It requires less expensive human resources than the traditional way of diagnosis. So the cost is less. AI needs nominal to zero rest time, and the maintenance expenditure is less than the wage of a radiologist. As a result, it is cost-effective. 
  **3. Consistency and Accuracy:**
      **a)	Standardization:** Sometimes, human interpretations are subjective. Automated systems provide consistent analysis, minimizing variability and subjectivity.
      **b)	Enhanced Detection:** Machine learning models can identify subtle patterns and features in X-rays that human eyes may miss.
  **4. Access to Care:**
      **a)	Remote Areas:** Experienced radiologists used to be available in big cities. Automated systems can be beneficial in remote or underserved areas.
      **b)	Telemedicine Integration:** Telemedicine is becoming popular due to easy accessibility and scarcity of physicians. Patients from isolated regions can benefit from automated X-ray analysis by integrating them into telemedicine platforms.
  **5. Handling Large Volumes of Data:**
      **a)	Efficient Data Processing:** Sometimes, the volumes of medical imaging data are enormous. Automated systems can handle them efficiently.
      **b)	Scalability:** AI can be used to scale the data from multiple sources simultaneously, making them suitable for large healthcare facilities and research purposes 
...
**Observations from the Filenames:** The filenames contain specific labels indicating whether the image shows a normal lung or pneumonia. The filenames for pneumonia cases include additional information, such as whether bacteria or a virus causes the pneumonia. 
**Challenges in Predicting Pneumonia:** The filenames suggest that pneumonia can be caused by bacteria or viruses, leading to variations in the appearance of pneumonia in X-rays. It can make it challenging to develop a single predictive model that can accurately detect all types of pneumonia.
...
**Q2 From looking at only 3 images do you see any attribute of the images that we may have to normalise before training a model?**

**Answer2:**

Consistency and standardization are crucial in preparing image data for deep learning. In my consideration, the following attributes of the images that we may have to normalize before training a model :
**1.	Image Size and Resolution:** To make all image dimensions consistent, we must resize them to an identical size. Identical dimension of images is essential for feeding into the deep learning model. Different image sizes can complicate the model training process.
**2.	Pixel Value Normalization:** We need to convert pixel values to a comparable scale to improve the model's convergence during training. 
**3.	Consistency in Image Format:**  We need to make the image format consistent. As we can see, the images are not in the same shapes and angles, so we need to adjust them.
...
Techniques to Handle Imbalanced Datasets in Medical Image Analysis:
   **1.	Data Augmentation:** We can create additional training examples by applying various transformations to existing images. Transformation may include rotation, scaling, flipping, and adding noise. 
   **2.	Class Weights:** We can increase the importance of the minority class by adjusting the weights of the classes during training. 
   **3.	Generative Models:** We can create new instances of the minority class using generative adversarial networks (GANs) or other generative models. This method can generate realistic and diverse images, and we can get a balance in the dataset.
   **4.	Oversampling:** We can repeat instances of the minority class to increase their representation in the dataset.
   **5.	Undersampling:** We can reduce the number of instances of the majority class to match the minority class. 
...
**ANswer6:**
Potential Drawbacks of Data Augmentation:
  **1.	Synthetic Nature of Augmented Data:** Augmented data are artificially generated through transformations. So, they may not perfectly capture the complexity and variability of real-world medical images. New data replicate existing ones, which potentially leads to overfitting.
  **2.	Increased Training Time:** Medical image data is intrinsically challenging" due to large image sizes and small datasets. However, data augmentation increases the size of the dataset, which can lead to longer training times and require more computational resources.
  **3.	Risk of Overfitting to Augmented Data:** The model might overfit to the specific types of transformations applied during augmentation
  **4.	Quality of Augmented Data:** Very long-tailed disease patterns in images indicate various presentations for conditions like pneumonia. This variety might not represented by poorly chosen augmentations, potentially leading to misleading training signals.
...
ImageNet is a large-scale image database organized according to the WordNet hierarchy (currently only the nouns) in which hundreds and thousands of images depict each node of the hierarchy.  It is a significant resource for computer vision research and has been instrumental in advancing the field.  It is useful, particularly in areas like object recognition and deep learning.  For the researcher's non-commercial use, the data is available for free. 
  **Dataset:** ImageNet contains 14,197,122 images . There are 21,841 synsets indexed in the ImageNet dataset, which can be considered as classes.  In ImageNet, each synset represents a unique concept or category.  Which provides a comprehensive dataset for training and evaluating machine learning models in various object recognition tasks.
  **Why might it be helpful to us:**
    **1. Large and Diverse Dataset for Training Models:** ImageNet offers more than 14 million images across 21,841 different synsets.  This huge amount of data can be invaluable for training robust machine learning models.  In addition, the diversity of images helps training models to recognize a wide range of visual features and improve their ability to generalize to unseen medical images.
    **2. Transfer Learning:** Models pre-trained on ImageNet can be fine-tuned for specific medical tasks.  Which saves time and computational resources.  Besides that, these models can effectively extract features from medical images.  As a result, various medical imaging applications such as tumour detection, organ segmentation, and disease classification use this model. 
    **3. Benchmarking and Evaluation:** As ImageNet provides a benchmark for evaluating the performance of new algorithms, it allows medical researchers to compare their methods against established standards.
    **4. State-of-the-Art Techniques:** Techniques such as convolutional neural networks (CNNs) and deep learning have revolutionized image analysis.  These models can be particularly useful in medical diagnostics.
    **5. Improving Diagnostic Tools:** Improved image recognition models can be used to develop automated diagnostic tools.  Enhanced image analysis can contribute to more precise treatments.
    **6. Cross-disciplinary Collaboration:** ImageNet encourages Collaboration between computer scientists and medical researchers, fostering interdisciplinary research.  As a result, we can achieve innovative solutions in healthcare.
...
**Issues with the assignment problem:** Evaluating the html file of the practical problem we can see that the test loss decreases sharply after epoch 2, indicating some improvement, while the initial epochs show very little change.  We can observe the model is overfitting to the training data and not generalizing well to the test data as training accuracy peaks and then drops, while the test accuracy remains flat. The model performs well on training data but poorly on validation data. It suggests that it has learned to memorize the training set rather than generalize from it. Additionally, there are large number of parameters in the fully connected layers compared to dataset size made the model complex. Which may lead to overfitting. 
Switching to the MobileNet architecture in your practical assignment can help address the current issues of overfitting, training inefficiency, and poor generalization. The advantages highlighted in the MobileNet publication—such as computational efficiency, reduced model size, flexibility, and competitive performance—make it a suitable and effective choice for your task of pneumonia detection from X-ray images.


**Q9. How many parameters does this network have? How does this compare to better performing networks available in torch?**

**Answer9:**
2,524,930 parameters. 

MobileNet is efficien, and it ideal for mobile and edge devices. We can compare with better-performing networks available in PyTorch:

1. ResNet: Though higher accuracy than MobileNet on large-scale datasets like ImageNet efficiency is less. It is more computationally intensive with higher parameter counts. And hence less suitable for mobile applications.
2. EfficientNet: It has slightly better accuracy than MobileNet for the smaller variants.
Comparable efficiency to MobileNet. However, it has larger variants offer higher accuracy at the cost of increased computational resources.
3. DenseNet: Due to dense connections, higher accuracy  which improve gradient flow and feature reuse.
But More computationally intensive than MobilNet.  It has fewer parameters than traditional convolutional networks.
4. ShuffleNet: Comparable in accuracy to MobileNet. Due to pointwise group convolutions and channel shuffle, often more efficient in terms of speed and resource usage.

For scenarios requiring a balance of accuracy and efficiency on mobile and edge devices mobileNet is excellent . However, for accuracy and avaialable resourses, networks like ResNet, EfficientNet, and DenseNet perform better.
